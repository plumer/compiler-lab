...
int main(int argc, char ** argv) {
	...
	if (argc == 3) {
		...
		outputfile = (char *)malloc( sizeof(char) * (strlen(argv[1]) + 1) );
		memset(outputfile, 0, sizeof(char) * (strlen(argv[1]) + 1) );
		strncpy(outputfile, argv[1], strlen(argv[1]) );
		char * p = outputfile + strlen(argv[1]) - 1;
		while (*p != '.') {
			p--;
			if (p < outputfile) {
				printf("file name format invalid\n");
				exit(-1);
			}
		}
		p[1] = 'i';
